...
from ..config import TICKER_SYMBOL, QUANTITY, STOP_PRICE_OFFSET, STOP_PRICE_LINK_TYPE, STOP_PRICE_LINK_BASIS, APP_KEY, APP_SECRET, CALLBACK_URL
from dotenv import load_dotenv
import schwabdev  # Assuming you have a Schwab API client called schwabdev
...
def authenticate():
    """Function to handle authentication with Schwab API"""
    response = client.authenticate()
    if response.ok:
        console.print("[bold green]Authenticated with Schwab API.[/bold green]")
    else:
        console.print(f"[bold red]Authentication failed. Status code: {response.status_code}, Response: {response.text}[/bold red]")

def get_account_hash():
    global account_hash
    console.print("[bold blue]Fetching linked accounts...[/bold blue]")
    authenticate()  # Ensure authentication is done
...
def place_buy_order_with_trailing_stop(ticker):
...
def cancel_and_replace_with_market_sell(ticker):
    # Cancel the trailing stop order
    if cancel_trailing_stop_order():
        # Wait for 1 second before placing the market sell order
        time.sleep(1)
...
def main():
    ...
    order_id, order_payload = place_buy_order_with_trailing_stop(TICKER_SYMBOL)
    
    # Wait for some time before canceling the trailing stop and placing a market sell order
    time.sleep(5)
    
    # Cancel the trailing stop and place a new market sell order
    cancel_and_replace_with_market_sell(TICKER_SYMBOL)
